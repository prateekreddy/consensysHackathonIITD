...
class CarrierFromRequestList extends React.Component {
 ...
 generateOrderList(){
   return this.state.orders.map((order,ind) => {
     return (
      <div style = {{textAlign : 'center', marginBottom : '20px', backgroundColor : 'white'}}>
        <PortOrderItem order = {order}/>
        <RaisedButton
          label = 'Reject'
          onClick = {() => this.handleRejectRequest(order.id)}
          backgroundColor = {'red'}
        />
        <RaisedButton
          label = 'Accept'
          onClick = {() => this.handleAcceptRequest(order.id)}
...
export default CarrierFromRequestList;
